...
export const dailyReminderJob = cron.schedule('* * * * *', async () => {
  try {
    ...
    for (const habit of habits) {
      ...
      if (habit.reminderTime === currentTime || habit.reminderTime === `${currentTime.split(':')[0]}:${(parseInt(currentTime.split(':')[1]) - 1).toString().padStart(2, '0')}`) {
        
        // Check if user has email reminders enabled
        if (habit.userId && habit.userId.settings?.emailReminders) {
          console.log(`[Reminder Job] Sending reminder for habit: ${habit.title} to user: ${habit.userId.name}`)
...
export const weeklyDigestJob = cron.schedule('0 9 * * 0', async () => {
  try {
