...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      logs.forEach((log) => {
        ...
        const habitIdStr =
          typeof log.habitId === "object" ? log.habitId._id : log.habitId;
  ...
  const handleHabitCheck = async (habitId, status) => {
    try {
      ...
      // Immediately refresh all dashboard data (including challenges)
      fetchDashboardData();
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-1 transition-colors duration-300">
          Welcome back, {user?.name || "User"}! Here's your eco-progress
          overview
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="card bg-gradient-to-br from-eco-green-500 to-eco-green-600 text-white">
          ...
          <p className="text-4xl font-bold mt-2">
            {stats.dailyEcoScore.toLocaleString()}
          </p>
        ...
        <div className="card bg-gradient-to-br from-blue-500 to-blue-600 text-white">
          ...
          <p className="text-xs mt-1 opacity-75">
            {Object.values(todayLogs).filter((s) => s === "done").length} of{" "}
        ...
        <div className="card bg-gradient-to-br from-purple-500 to-purple-600 text-white">
          ...
          <p className="text-xs mt-1 opacity-75">
            {stats.activeChallenges > 0 ? "In progress" : "Join a challenge!"}
          </p>
        </div>
      </div>
        ...
        {todayHabits.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-500 dark:text-gray-400">
              No habits yet. Create your first eco-habit!
            </p>
              ...
              >
                <div className="flex items-center space-x-3 flex-1">
                  ...
                  <span className="text-sm text-gray-500 dark:text-gray-400">
                    ...
                    {habit.category === "Food" && "🍃"}
                    {habit.category === "Waste" && "♻️"} {habit.category}
                  </span>
                </div>
          ...
          {challenges.length === 0 ? (
            ...
            <div className="space-y-3">
              {challenges.slice(0, 2).map((progress) => (
                <div
                  key={progress._id}
                  className="p-4 bg-eco-green-50 dark:bg-eco-green-900/30 rounded-lg transition-colors duration-300"
                >
                      ...
                      style={{
                        width: `${
                          (progress.completedDays /
                            (progress.challengeId?.durationDays || 7)) *
                          100
                        }%`,
                      }}
                    ></div>
